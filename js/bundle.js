...
var levelSelection = exports.levelSelection = function levelSelection(gitState) {
...
var levelStructure = exports.levelStructure = function levelStructure(gitState) {
  switch (gitState.level) {
    ...
    case 4:
      ...
      break;
    case 4.1:
      gitState.instructions = "You can commit files in your staging area with the command: <span class='bg'>git commit</span></p><p>As we know, a <span class='bg'>commit</span> is like a log in your git. Well, every log entry needs to have a heading, a context of what the log is about. Simlarly, a <span class='bg'>commit</span> requires a message. You can add a message by adding <span class='bg'>-m 'message here'</span> to the commit command</p><p>Try it yourself, run <span class='bg'>git commit -m 'some message here'</span>, make sure your message describes what you just did.</p>";
      gitState.fileStructure = allStagedAnimals;
      gitState.commitHistory = [];
      break;
    case 5:
...
var gitFirstAnimals = Object.assign({}, {
  '.git': {
    'status': 'ignored',
    'details': 'folder'
  }
...
var stagedFirstAnimals = Object.assign({}, gitFirstAnimals, {
  'bear': {
    'status': 'staged',
    'details': 'bear'
  },
...
var allStagedAnimals = Object.assign({}, stagedFirstAnimals, { 'cat': {
    'status': 'staged',
    'details': 'cat'
  },
  'dog': {
...
var allCommittedAnimals = Object.assign({}, allStagedAnimals, {
  'bear': {
    'status': 'committed',
    'details': 'bear'
  },
...
var _terminal = __webpack_require__(15);
...
var _union = __webpack_require__(18);
...
var rmCommand = function rmCommand(gitState, command) {
  ...
  if (command.slice(3, 11) === '-rf .git') {
    ...
    var structure = gitState.branch.status && gitState.branch.checkout ? gitState.branch : gitState;
    delete structure.fileStructure[command.slice(3)];
    return 'deleted file';
  }
};

var gitCommand = function gitCommand(gitState) {
  var command = gitState.currentCommand;
  var structure = gitState.branch.status && gitState.branch.checkout ? gitState.branch : gitState;
  if (!gitState.initialized) {
    ...
    if (command.slice(4, 8) === 'init') {
      ...
      if (command.slice(11, 29) === '--global user.name' || command.slice(11, 20) === 'user.name') {
        gitState.username = command.split(' ')[2] === '--global' ? command.split(' ').slice(4).join(' ') : command.split(' ').slice(3).join(' ').slice(1, -1);
        if (gitState.level === 2) {
          gitState.level++;
        }
...
var gitMerge = function gitMerge(gitState, command) {
  if (command === '') {
...
var logHistory = function logHistory(gitState) {
  if (gitState.commitHistory.length) {
    var commits = '';
...
var gitCheckout = function gitCheckout(gitState, command) {
  if (Object.keys(gitState.fileStructure).includes(command.split(' ')[0])) {
    var result = '';
    command.split(' ').forEach(function (file) {
...
var gitDiff = function gitDiff(gitState, command) {
  var result = '';
  var files = [];
  if (command === '') {
    files = _lodash2.default.union(Object.keys(gitState.commitHistory[0].fileStructure), Object.keys(gitState.fileStructure));
  } else {
    files = command.split(' ');
  }
...
var stageFiles = function stageFiles(gitState, files) {
  var missing = [];
  files.forEach(function (file) {
    ...
    if (gitState.commitHistory.length) {
      Object.keys(gitState.commitHistory[0].fileStructure).forEach(function (file) {
  ...
  var staged = Object.keys(gitState.fileStructure).filter(function (file) {
    return gitState.fileStructure[file].status === 'staged';
  });
...
var commitFiles = function commitFiles(gitState, message) {
  if (message.slice(1, -1) === '') {
    ...
    Object.keys(gitState.fileStructure).forEach(function (file) {
...
var SetCache = __webpack_require__(42),
    ...
    createSet = __webpack_require__(73),
    setToArray = __webpack_require__(13);
...
var render = function render(gitState) {
  ...
  if (gitState.branch.status) {
    Object.keys(gitState.branch.fileStructure).forEach(function (file) {
